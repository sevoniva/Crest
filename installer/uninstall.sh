...
if test -n "$(docker images | grep -E 'ghcr.io/sevoniva/dataease-2.10.22')"; then
   echo "清理 DataEase 镜像"
   docker rmi $(docker images | grep -E "ghcr.io/sevoniva/dataease-2.10.22" | awk -F' ' '{print $1":"$2}')
fi
