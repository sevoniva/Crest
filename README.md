...
Crest 支持基于 OIDC / OAuth2 授权码模式的单点登录。配置入口位于“系统设置 / 单点登录”，适用于能够提供授权端点、令牌端点和 UserInfo 端点的企业身份提供方。

关键规则：

- Client Secret 使用运行时 AES 配置加密保存；
- 回调阶段只下发一次性票据，Crest token 不出现在回调 URL 中；
- 用户按外部唯一标识和账号绑定，可按配置自动创建普通账号；
- 关闭本地登录后，管理员仍可通过 `/#/admin-login` 处理应急运维；
- 生产环境建议启用 HTTPS，并在代理层透传 `X-Forwarded-*` 头。

配置、字段映射和排障说明见 [docs/sso.md](./docs/sso.md)。
...
Crest 1.3 补齐了社区版可用的平台管理能力，包括用户、组织、角色、菜单权限和业务资源权限。权限判断在后端执行，前端只负责菜单、按钮和授权界面的展示。

平台管理能力覆盖：

- 用户新建、编辑、删除、启停、批量删除和批量导入；
- 用户绑定一个或多个角色；
- 多级组织树和用户默认组织；
- 数据源、数据集、仪表盘、数据大屏按组织、创建人和授权关系过滤；
- 系统设置中的组织管理、角色管理和权限管理。

设计口径、数据库变更和影响文件见 [docs/platform-management.md](./docs/platform-management.md)。
...
| [docs/sso.md](./docs/sso.md) | 单点登录配置、登录流程、用户映射和运维要求 |
| [docs/platform-management.md](./docs/platform-management.md) | 平台管理、组织角色、菜单权限和资源权限的设计口径 |
