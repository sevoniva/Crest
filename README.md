...
Crest 支持通用 OIDC / OAuth2 Authorization Code 单点登录。配置入口位于“系统设置 / 单点登录”，可接入提供授权端点、令牌端点和 UserInfo 端点的企业身份提供方。

关键规则：

- Client Secret 使用 `CREST_AES_KEY` 和 `CREST_AES_IV` 加密保存；
- 登录回调使用一次性 ticket 换 Crest token，token 不直接出现在回调 URL 中；
- 用户按外部唯一标识和账号绑定，首次登录可自动创建普通用户；
- 关闭普通本地登录后，管理员仍可通过 `/#/admin-login` 应急登录；
- 生产环境建议开启 HTTPS，并在代理层透传 `X-Forwarded-*` 头。
