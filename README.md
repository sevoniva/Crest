...
- 精简内部版前端功能边界，下线 SQLBot、模板市场、工具箱、消息中心、移动端、公开分享、地图类图表创建入口和 demo 模板资源。
...
- 公开分享入口已下线，历史分享链接不作为内部版支持范围；
- 移动端独立页面不再构建，PC 端嵌入和仪表板预览继续保留；
- 地图类图表不再出现在新建图表列表中，内部轻量构建不再打包 L7、L7Plot 和 Mapbox 运行时；
- 模板市场相关前端入口已删除，系统内置模板管理入口按主链路兼容保留。
...
默认前端构建跳过 Vite 阶段的 ESLint、Stylelint 和 gzip 预压缩，减少本地与 CI 打包耗时。提交前需要完整检查时执行 `npm run lint:check` 或 `npm run build:base:strict`。
