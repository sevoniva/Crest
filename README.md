...
新安装环境会初始化一个干净的 `Demo` 数据源，并放入一组茶饮经营血缘样例表和数据集。样例包含订单、订单明细、门店、商品、会员、优惠券、退款和原料成本 8 张表，用来验证字段筛选、跨表关联和计算字段血缘。系统不预置仪表板成品，避免新环境里出现过期资源或无关示例。
...
安装脚本里的 MySQL `init.sql` 只负责创建数据库，不承载业务元数据。新增初始化内容时，应追加 Flyway 迁移并重新做空库安装验证。
