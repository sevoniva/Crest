...
这是一个 Crest 开源 BI 版本，主要用于支持 OceanBase Oracle 模式租户，并补充 OB Oracle 数据集缓存同步能力。

## 来源与许可证

本仓库基于 [DataEase 官方仓库](https://github.com/dataease/dataease.git) fork 修改，基线版本为 DataEase 2.10.22，并在此基础上维护 Crest 品牌和 OceanBase Oracle 支持。
...
- OB Oracle 数据集支持 Crest 数据集结果缓存。
- 支持缓存全量同步、增量同步、定时同步和手动同步。
- 数据集删除时清理对应缓存表、同步任务和同步日志。
- 增加内部轻量模式，默认隐藏商业版和非内部使用入口。
- 精简内部版前端功能边界，下线 SQLBot、模板市场、工具箱、消息中心、移动端、商业插件、地图类图表、地图接口、离线地图资源和 demo 模板资源。
- 保留仪表板和数据大屏分享能力。
- 新增单独的 `数据血缘` 菜单，支持数据源、数据集、仪表板和数据大屏的字段级血缘查看。
- 数据血缘支持按物理表选择字段，查询字段完整上下游。
- 数据源和数据集删除前会基于血缘关系检查下游影响。
- 内部版品牌资源替换为 Crest，包括顶部 logo、登录页 logo 和浏览器图标。
- 离线安装默认只部署 Crest 与 MySQL，同步任务服务和 Playwright 按需外接。
- 部署镜像和安装模板改为使用 GitHub Container Registry。
- 构建依赖改为使用公开 Maven 坐标和官方 npm registry。

## OB Oracle 数据源

本 fork 适配 OceanBase 数据库 Oracle 模式租户。数据源配置方式与 Crest 其他 JDBC 数据源保持一致。
...
| 场景 | Host | Port | Username | 说明 |
| --- | --- | --- | --- | --- |
| OBServer 直连 | `127.0.0.1` | `2881` | `test@obora` | 验证直连 observer |
| OBProxy/ODP | `127.0.0.1` | `2883` | `test@obora#obdemo` | 验证代理模式和 cluster 解析 |

两种方式都需要覆盖连通性校验、表字段读取、字段备注展示、数据预览、数据集保存、缓存同步和仪表板查看。
...
生产环境建议单独创建 Crest 查询账号，并在 OceanBase 侧只授予所需的 `SELECT` 权限。不要使用拥有 DDL、DML 或管理员权限的业务账号。

## 字段备注

OB Oracle 数据集取字段时会读取 Oracle 模式字段注释。字段有备注时，Crest 展示备注；没有备注时回退为字段名。

这样创建数据集和制作图表时可以直接看到业务含义，不需要用户再根据技术字段名反查含义。

## 数据集缓存同步

这里实现的是“数据集结果缓存”，不是整库同步，也不是把源端业务表完整复制到 Crest。

Crest 会按数据集建模规则生成查询 SQL，执行后把查询结果写入内部引擎表。缓存就绪后，数据预览和仪表板读取内部缓存表，减少对 OB Oracle 源库的直接查询压力。
...
血缘图从 Crest 已保存的元数据生成，不直接扫描业务库。当前链路覆盖：
...
- `全局`：查看资源级整体依赖；
- `数据源`：查看某个数据源进入 Crest 后形成的字段级链路；
- `数据集`：反查数据集上游表字段，并查看下游图表和展示资源；
- `仪表板/大屏`：从展示资源反查使用到的图表、数据集和源字段。

字段级查询按“先选表、再选字段”的方式使用。选择字段后，图谱会收敛到该字段的完整上下游，适合排查字段口径、修改影响和下线风险。

本地 Demo 环境默认选中 Crest 内置 `Demo` 数据源。示例数据里可以看到 `demo_tea_order`、`demo_tea_material` 两张表，以及茶饮销售看板的字段链路。

完整说明见 [docs/data-lineage.md](./docs/data-lineage.md)。
...
- 默认入口保留工作台、仪表板、数据大屏、分享、数据准备、数据集、数据源、数据血缘、系统参数和字体；
- 默认安装包只启动 Crest 与 MySQL。
...
- 分享功能保留，分享链接、访问校验和 ticket 管理接口继续注册。代码中仍会出现 `xpack_share`、`XpackShare*`、`io.dataease.api.xpack.share` 等命名，这是为了兼容原有表结构、接口路径、Mapper 和历史分享链接，不代表重新引入商业插件。不要直接改名；如需改名，必须同时提供历史数据和历史链接的迁移方案；
- 移动端独立页面不再构建，PC 端嵌入和仪表板预览继续保留；
- 地图类图表、地图接口、离线地图文件和 L7、L7Plot、Mapbox 运行时均不作为内部版支持范围。数据库实体中可能保留少量历史字段，只用于读取旧图表配置，不作为功能入口；
...
模板中的 Crest、MySQL、Playwright API 和同步任务镜像均使用 `ghcr.io/sevoniva/crest*` 命名空间。

从 0 开始部署时，推荐流程如下：

```bash
# 1. 准备安装目录
cp -R installer/dataease /opt/dataease
cd /opt/dataease

# 2. 按实际环境修改 install.conf
vi install.conf
...
如果使用自己构建的镜像，只需要在 `install.conf` 或 compose 文件中把 Crest 镜像标签改成目标版本。内部轻量版默认不启动同步任务服务和外部 Playwright；需要这些服务时再按需打开。
...
docker-compose \
  ...
  up -d --no-deps dataease
```

`--no-deps dataease` 表示只重新创建 Crest 容器，不重启 MySQL、Doris 等依赖服务。回滚时把镜像标签改回旧版本，再执行同一条命令即可。
...
默认前端构建跳过 Vite 阶段的 ESLint、Stylelint 和 gzip 预压缩，减少本地与 CI 打包耗时。提交前需要完整检查时执行 `pnpm run lint:check` 或 `pnpm run build:base:strict`。构建完成后执行 `pnpm run build:lite:check`，用于确认源码、依赖声明和构建产物中没有重新引入地图接口、Mapbox、L7、L7Plot、Maplibre 或 PMTiles。后端从根目录构建 `:core-backend`，Maven 会同时编译本仓库的 `sdk` 源码，不需要从外部 Maven 仓库拉取 Crest API 包。
...
- 涉及数据血缘的改动需要回归全局、数据源、数据集、仪表板/大屏、表字段筛选和删除影响检查。
