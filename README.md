...
血缘图从 DataEase 已保存的元数据生成，不直接扫描业务库。当前链路覆盖：
...
- `全局`：查看资源级整体依赖；
- `数据源`：查看某个数据源进入 DataEase 后形成的字段级链路；
- `数据集`：反查数据集上游表字段，并查看下游图表和展示资源；
- `仪表板/大屏`：从展示资源反查使用到的图表、数据集和源字段。

字段级查询按“先选表、再选字段”的方式使用。选择字段后，图谱会收敛到该字段的完整上下游，适合排查字段口径、修改影响和下线风险。

本地 Demo 环境默认选中 DataEase 内置 `Demo` 数据源。示例数据里可以看到 `demo_tea_order`、`demo_tea_material` 两张表，以及茶饮销售看板的字段链路。
...
如果使用自己构建的镜像，只需要在 `install.conf` 或 compose 文件中把 DataEase 镜像标签改成目标版本。内部轻量版默认不启动同步任务服务和外部 Playwright；需要这些服务时再按需打开。

已有环境升级时，建议先备份数据库和安装目录，再只替换 DataEase 服务镜像：
...
`--no-deps dataease` 表示只重新创建 DataEase 容器，不重启 MySQL、Doris 等依赖服务。回滚时把镜像标签改回旧版本，再执行同一条命令即可。
